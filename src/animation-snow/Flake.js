...
/*
const FlakeHook = ({radius, x, y, density}) => {
  const [position, setPosition] = useState(() => new Animated.ValueXY({x, y}));
  let angle = 0;
  useEffect(() => {
    animate();
  });
  const animate = () => {
    ...
    angle += 0.02;
    let newX = x + Math.sin(angle) * 10;
    let newY = y + Math.cos(angle + density) + 10 + radius / 2;
